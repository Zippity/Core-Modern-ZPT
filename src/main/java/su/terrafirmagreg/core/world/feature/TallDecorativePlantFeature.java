    ...
    public boolean place(FeaturePlaceContext<TallDecorativePlantConfig> context) {
        ...
        if (!(context.config().block() instanceof TallDecorativePlantBlock)) {
            TFGCore.LOGGER.error(
                    "TallDecorativePlantFeature was passed a block that isn't a TallDecorativePlantBlock! Was: {}",
        ...
        int height = context.random().nextIntBetweenInclusive(context.config().minHeight(),
                context.config().maxHeight());
        ...
        for (int i = 0; i < Math.max(plantHeight, height); i++) {
            ...
            if (!currentFluid.isEmpty()) {
                state = state.setValue(TallDecorativePlantBlock.FLUID,
                        TallDecorativePlantBlock.FLUID.keyForOrEmpty(currentFluid.getType()));
            }
